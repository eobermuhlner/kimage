...
kimage(0.1) {
    name = "remove-background-gradient-manual2"
    title = "Remove the background by subtracting an interpolated gradient"
    description = """
                Calculates an interpolated background image from 2 manually chosen fix points and removes it.
    ...
    single {
        ...
        val points = listOf(x1 to y1, x2 to y2)
