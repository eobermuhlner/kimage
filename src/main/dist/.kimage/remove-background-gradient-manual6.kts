...
kimage(0.1) {
    name = "remove-background-gradient-manual6"
    title = "Remove the background by subtracting an interpolated gradient"
    description = """
                Calculates an interpolated background image from 5 manually chosen fix points and removes it.
    ...
    single {
        ...
        val x4: Int by arguments
        val y4: Int by arguments
        val x5: Int by arguments
        val y5: Int by arguments
        val x6: Int by arguments
        val y6: Int by arguments
        val medianRadius: Int by arguments

        val points = listOf(x1 to y1, x2 to y2, x3 to y3, x4 to y4, x5 to y5, x6 to y6)
