    ...
    override fun align(base: Image, image: Image, centerX: Int, centerY: Int, maxOffset: Int): ImageAligner.Alignment {
        ...
        for (dy in -maxOffset .. maxOffset) {
            for (dx in -maxOffset .. maxOffset) {
                val x = centerX + dx
                val y = centerY + dy
                val error1 = baseCropped1.averageError(image.crop(x-fastRadiusX, y-fastRadiusY, fastRadiusX*2+1, fastRadiusY+1, false))
                if (error1 < bestError1 * fastErrorThreshold) {
                    //println("Error1: $dx, $dy : $error1")
                    val error2 = baseCropped2.averageError(image.crop(x-radiusX, y-radiusY, radiusX*2+1, radiusY*2+1, false))
